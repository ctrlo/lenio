...
SELECT me.id, me.name, me.description, me.period_unit, me.period_qty, me.global, MAX( ticket.completed ) AS completed, MAX( ticket.planned ) AS planned, SUM( ticket.cost_planned ) AS cost_planned, SUM( ticket.cost_actual ) AS cost_actual, site_task.id AS site_task_id, MIN(IFNULL(ticket_id, -1)) AS is_extant, site_task.site_id AS site_id
FROM task me
